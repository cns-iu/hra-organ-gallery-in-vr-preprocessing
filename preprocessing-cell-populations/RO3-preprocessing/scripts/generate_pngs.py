...
Creates both single-AS and combined-AS charts
...
def create_combined_as_chart(df, tool, sex, output_dir):
    """
    ...
    # Get AS IDs
    as_id_map = df_subset.groupby('as_label')['as'].first().apply(extract_as_id)
    as_ids = [as_id_map[label] for label in as_order]
    ...
    fig.update_layout(
        barmode='stack',
        xaxis_title='Anatomical Structure',
        yaxis_title='Cell Count',
        xaxis_tickangle=config.X_AXIS_ANGLE,
        margin=dict(l=80, r=180, t=100, b=120),
        legend=dict(
    ...
    # Generate filename
    ref_organ_id = get_ref_organ_id(sex)
    as_id_string = '-'.join(as_ids)
    filename = f"{ref_organ_id}--{tool}--{as_id_string}.png"
    filepath = os.path.join(output_dir, filename)
...
def main():
    ...
    print(f"\n" + "=" * 60)
    print("📈 COMBINED ANATOMICAL STRUCTURE CHARTS")
    print("=" * 60)
    
    for tool in tools:
        for sex in sexes:
            print(f"\n   {tool} / {sex}:")
            filepath = create_combined_as_chart(df, tool, sex, config.OUTPUT_DIR)
            if filepath:
                generated_files.append(filepath)
